...
export class OrderbookJS extends RainJS {
   ...
   */
   protected _CONTEXT_: ApplyOpFn = {
    [OrderbookContext.OrderHash]: (state: StateJS, operand: number, data?: any) => {
      if(data && data.context != undefined) {
        state.stack.push(
          BigNumber.from(
            data.context[OrderbookContext.OrderHash]
          )
        )
      }
    ...
    [OrderbookContext.CounterParty]: (state: StateJS, operand: number, data?: any) => {
      if(data && data.context != undefined) {
        state.stack.push(
          BigNumber.from(
            data.context[OrderbookContext.CounterParty]
          )
        )
      }
      else throw new Error("Undefined buy units")
   ...
   * @remark Orderbook doesnt have any STORAGE opcode by default and in its contract level,
