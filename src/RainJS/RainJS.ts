...
export class RainJS {
   ...
   * because the RainJS opcode functions should change after the an instance has be created.
   */
  ...
  protected async dispatch (
    ...
    if (opcode == RainJS.Opcodes.SKIP) {
      throw new Error("SKIP is no longer available")
    }
    else if (opcode == RainJS.Opcodes.VAL) {
      if (!(operand >> 7)) {
        ...
        if (state.argsArr[operand & 127] != undefined) {
          state.stack.push(state.argsArr[operand & 127])
        }
        else throw new Error("out-of-bound arguments")
      }
    }
    else if (opcode == RainJS.Opcodes.DUP) {
      state.stack.push(state.stack[operand]);
    }
    else if (opcode == RainJS.Opcodes.ZIPMAP) {
      ...
      if (items_.length == numberOfVals_ + 1) {
        for (let i = 0; i < loopSize_; i++) {
          state.argsArr.splice(-(numberOfVals_ + 1));
    ...
    else if (opcode == RainJS.Opcodes.DEBUG) {
      console.log(state.stack);
    }
    else if (opcode == RainJS.Opcodes.BLOCK_NUMBER) {
    ...
    else if (opcode == RainJS.Opcodes.BLOCK_TIMESTAMP) {
      if (this.provider != undefined) {
        state.stack.push(
          BigNumber.from(
            (
    ...
    else if (opcode == RainJS.Opcodes.SENDER) {
    ...
    else if (opcode == RainJS.Opcodes.THIS_ADDRESS) {
    ...
    else if (opcode == RainJS.Opcodes.SCALE18_MUL) {
    ...
    else if (opcode == RainJS.Opcodes.SCALE18_DIV) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ != undefined) {
        state.stack.push(
    ...
    else if (opcode == RainJS.Opcodes.SCALE18) {
    ...
    else if (opcode == RainJS.Opcodes.SCALEN) {
      const item_ = state.stack.pop();
      if (item_ != undefined) {
        state.stack.push(
          operand <= 18   
    ...
    else if (opcode == RainJS.Opcodes.SCALE_BY) {
      const item_ = state.stack.pop();
    ...
    else if (opcode == RainJS.Opcodes.SCALE18_ONE) {
      state.stack.push(BigNumber.from("1000000000000000000"));
    }
    else if (opcode == RainJS.Opcodes.SCALE18_DECIMALS) {
      state.stack.push(BigNumber.from(18));
    }
    else if (opcode == RainJS.Opcodes.ADD) {
      let _item;
    ...
    else if (opcode == RainJS.Opcodes.SATURATING_ADD) {
      ...
      state.stack.push(await _accumulator);
    }
    else if (opcode == RainJS.Opcodes.SUB) {
      const items_ = state.stack.splice(-operand);
    ...
    else if (opcode == RainJS.Opcodes.SATURATING_SUB) {
      const items_ = state.stack.splice(-operand);
      let _accumulator = items_.shift();
      let _item;
      if (_accumulator != undefined) {
    ...
    else if (opcode == RainJS.Opcodes.MUL) {
    ...
    else if (opcode == RainJS.Opcodes.SATURATING_MUL) {
      const items_ = state.stack.splice(-operand);
      let _accumulator = ethers.constants.One;
      let _item;
      for (let i = 0; i < operand; i++) {
    ...
    else if (opcode == RainJS.Opcodes.DIV) {
      const items_ = state.stack.splice(-operand);
      let _accumulator = items_.shift();
      let _item;
      if (_accumulator != undefined) {
    ...
    else if (opcode == RainJS.Opcodes.MOD) {
      const items_ = state.stack.splice(-operand);
      let _accumulator = items_.shift();
      let _item;
      if (_accumulator != undefined) {
    ...
    else if (opcode == RainJS.Opcodes.EXP) {
      const items_ = state.stack.splice(-operand);
      let _accumulator = items_.shift();
      let _item;
      if (_accumulator != undefined) {
    ...
    else if (opcode == RainJS.Opcodes.MIN) {
      const items_ = state.stack.splice(-operand);
    ...
    else if (opcode == RainJS.Opcodes.MAX) {
      const items_ = state.stack.splice(-operand);
      if (items_.length = operand) {
        state.stack.push(
    ...
    else if (opcode == RainJS.Opcodes.ISZERO) {
      const item_ = state.stack.pop();
      if (item_ != undefined) { 
        state.stack.push(
          item_.isZero() 
    ...
    else if (opcode == RainJS.Opcodes.EAGER_IF) {
    ...
    else if (opcode == RainJS.Opcodes.EQUAL_TO) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ != undefined) {
        state.stack.push(
    ...
    else if (opcode == RainJS.Opcodes.LESS_THAN) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ != undefined) {
        state.stack.push(
    ...
    else if (opcode == RainJS.Opcodes.GREATER_THAN) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ != undefined) {
        state.stack.push(
    ...
    else if (opcode == RainJS.Opcodes.EVERY) {
      const items_ = state.stack.splice(-operand);
      let _check;
      let _item;
      for (let i = 0; i < operand; i++) {
    ...
    else if (opcode == RainJS.Opcodes.ANY) {
      const items_ = state.stack.splice(-operand);
      let _check;
      let _item;
      for (let i = 0; i < operand; i++) {
    ...
    else if (opcode == RainJS.Opcodes.REPORT) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
    ...
    else if (opcode == RainJS.Opcodes.NEVER) {
      state.stack.push(ethers.constants.MaxUint256)
    }
    else if (opcode == RainJS.Opcodes.ALWAYS) {
      state.stack.push(ethers.constants.Zero)
    }
    else if (opcode == RainJS.Opcodes.SATURATING_DIFF) {
    ...
    else if (opcode == RainJS.Opcodes.UPDATE_BLOCKS_FOR_TIER_RANGE) {
    ...
    else if (opcode == RainJS.Opcodes.SELECT_LTE) {
    ...
    else if (opcode == RainJS.Opcodes.IERC20_BALANCE_OF) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ && this.signer != undefined) {
        const account_ = "0x" + item2_.toHexString().substring(2).padStart(64, "0");
    ...
    else if (opcode == RainJS.Opcodes.IERC20_TOTAL_SUPPLY) {
      const item_ = state.stack.pop();
    ...
    else if (opcode == RainJS.Opcodes.IERC721_BALANCE_OF) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ && this.signer != undefined) {
        const account_ = "0x" + item2_.toHexString().substring(2).padStart(64, "0");
    ...
    else if (opcode == RainJS.Opcodes.IERC721_OWNER_OF) {
      const item2_ = state.stack.pop();
      const item1_ = state.stack.pop();
      if (item1_ && item2_ && this.signer != undefined) {
    ...
    else if (opcode == RainJS.Opcodes.IERC1155_BALANCE_OF) {
    ...
    else if (opcode == RainJS.Opcodes.IERC1155_BALANCE_OF_BATCH) {
