...
export class Trust {
  ...
  public static async fromAddress(
    ...
    const reserve = await Reserve.fromAddress(
      addresses.reserveERC20,
      signerOrProvider,
      chainId
    );
    const redeemable = await Redeemable.fromAddress(
      addresses.redeemableERC20,
      signerOrProvider,
      chainId
    );
    const redeemableERC20Pool = await RedeemableERC20Pool.fromAddress(
      addresses.redeemableERC20Pool,
      signerOrProvider,
      chainId
    );
    const seeder = await SeedERC20.fromAddress(
      addresses.seeder,
      signerOrProvider,
      chainId
    );
    const crp =
      addresses.crp !== ethers.constants.AddressZero
        ? await CRP.fromAddress(addresses.crp, signerOrProvider, chainId)
        : null;
    const pool =
      addresses.pool !== ethers.constants.AddressZero
        ? await Pool.fromAddress(addresses.pool, signerOrProvider, chainId)
        : null;

    const tier = ReadOnlyTier__factory.connect(
      addresses.tier,
      signerOrProvider
    );
  ...
  public static async fromTransactionReceipt(
    ...
    if (trustAddress === undefined) {
      throw Error('Event "NewContract" is not in transaction receipt.');
    }

    return this.fromAddress(trustAddress, signerOrProvider, chainId);
  }

  public static async createTrust(
    config: Config,
    signer: Signer,
    chainId: number
  ): Promise<ContractTransaction> {
    const trustFactory = TrustFactory__factory.connect(
      AddressBook.getFactoryAddressFromChainId(chainId),
      signer
    );
